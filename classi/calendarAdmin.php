...
class CalendarAdmin {  
    ...
    */
    private function _showDay($cellNumber)
    {
		...
		$data = date('Y-m-d',strtotime($this->currentYear.'-'.$this->currentMonth.'-'.($this->currentDay)));
		
		$sql = "SELECT SUM(num_partecipanti) AS Totale FROM prenotazioni WHERE giorno = '" . $data . "';";
		...
		{
        	$chiusura = $this->_isClosed($this->currentDate);
            ...
            else if($chiusura < 0)
			{
                if($chiusura == -1)
					...
					$string .= '<div id="li-'.$this->currentDate.'" class="col-xs-1 text-center" style="background-color: rgb(255, 3, 30); ">'.$cellContent.'</div>';
			}
